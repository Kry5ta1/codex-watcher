...
export function makeNudge(windows, resetCount, resetUrgencies = []) {
  ...
  const fiveHour = windows.find((window) => window.kind === "fiveHour");
  const fiveHourRemaining = fiveHour?.remainingPercent;
  const fiveHourReset = fiveHour?.resetAfterSeconds;
  const weeklyResetSeconds = weekly.resetAfterSeconds;
  const fiveHourCritical =
    Number.isFinite(fiveHourRemaining) && fiveHourRemaining <= 10;
  const fiveHourLow =
    Number.isFinite(fiveHourRemaining) && fiveHourRemaining <= 25;
  const fiveHourHealthy =
    !Number.isFinite(fiveHourRemaining) || fiveHourRemaining >= 35;
  const fiveHourResetSoon =
    Number.isFinite(fiveHourReset) && fiveHourReset <= 90 * 60;
  const fiveHourResetLater =
    Number.isFinite(fiveHourReset) && fiveHourReset > 90 * 60;
  const weeklyCritical = weeklyRemaining <= 15;
  const weeklyLow = weeklyRemaining <= 25;
  const weeklyHealthy = weeklyRemaining >= 45;
  ...
  if (
    ...
    return {
      ...
      title: "先等 5 小时窗口恢复",
      message:
        `5 小时窗口将在 ${duration(fiveHourReset)} 后恢复。先等短窗口恢复，不必急着花掉储备重置。`,
      detail: `5 小时窗口将在 ${duration(fiveHourReset)} 后重置`
    };
  }
  ...
  if (
    ...
    weeklyHealthy &&
    fiveHourResetLater &&
    fiveHourReset <= 3 * 3600
  ) {
    return {
      tier: "deadline",
      title: "按截止时间决定",
      message:
        `短窗口还要 ${duration(fiveHourReset)} 才恢复。赶截止时间可以用 1 个重置；不赶时间就等它自然恢复。`,
      detail: `5 小时窗口将在 ${duration(fiveHourReset)} 后重置`
    };
  }
  ...
  if (
    ...
    return {
      ...
      title: "截止时间优先",
      message:
        `5 小时窗口恢复还要 ${duration(fiveHourReset)}。只有重要任务被短窗口卡住时才用重置，普通任务先排队。`,
      detail: `5 小时窗口将在 ${duration(fiveHourReset)} 后重置`
    };
  }

  if (fiveHourLow && resetCount >= 1) {
    return {
      tier: "useIfBlocked",
      title: "短窗口偏低，重置只给阻塞任务",
      message:
        "先减少连续大任务；如果当前工作已经被卡住，可以使用 1 个储备重置。",
      detail: "5 小时额度偏低"
    };
  }
  ...
  if (hasSoonReset && (fiveHourLow || weeklyLow)) {
    return {
      tier: "expiringReset",
      title: "有重置即将到期",
      message:
  ...
  if (weeklyPlenty && fiveHourHealthy) {
    return {
      tier: "hold",
      title: "额度健康，先不动重置",
      message:
...
function displayWindow(raw, fallbackID, limitReached) {
  ...
  const kind =
    fallbackID === "primary" || (seconds >= 14400 && seconds <= 21600)
      ? "fiveHour"
      : fallbackID === "secondary" || (seconds >= 518400 && seconds <= 864000)
        ? "weekly"
        : "generic";

  return {
    id:
      kind === "fiveHour" ? "five-hour" : kind === "weekly" ? "weekly" : fallbackID,
    kind,
    limitReached,
    limitWindowSeconds,
    remainingPercent:
